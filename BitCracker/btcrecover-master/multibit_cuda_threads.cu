 ...
 * MultiBit check (3x MD5 + AES + base58) one thread per password. A producer
 * thread fills the next batch while the GPU works on the current one. About
 * 11M pw/s on an RTX 2060 for search46-style token lists.
 *
...
HD void md5_compress(uint32_t* st, uint32_t* blk) {
...
HD void md5(const uint8_t* data, uint32_t len, uint8_t* digest) {
...
HD bool all_b58(const uint8_t* buf, int len) {
    for (int i=0;i<len;i++) {
        uint8_t c=buf[i];
...
static uint32_t h_TD0[256], h_TD1[256], h_TD2[256], h_TD3[256];
...
static void build_and_upload_tables() {
    for (int x=0;x<256;x++) {
        ...
        h_TD0[x]=((uint32_t)gmul(14,s)<<24)|((uint32_t)gmul(9,s)<<16)|((uint32_t)gmul(13,s)<<8)|gmul(11,s);
        h_TD1[x]=(h_TD0[x]>>8)|(h_TD0[x]<<24);
        h_TD2[x]=(h_TD1[x]>>8)|(h_TD1[x]<<24);
        h_TD3[x]=(h_TD2[x]>>8)|(h_TD2[x]<<24);
    }
    CUDA_CHECK(cudaMemcpyToSymbol(c_TD0,     h_TD0,     1024));
    CUDA_CHECK(cudaMemcpyToSymbol(c_TD1,     h_TD1,     1024));
    CUDA_CHECK(cudaMemcpyToSymbol(c_TD2,     h_TD2,     1024));
    CUDA_CHECK(cudaMemcpyToSymbol(c_TD3,     h_TD3,     1024));
    CUDA_CHECK(cudaMemcpyToSymbol(c_SBOX,    h_SBOX,    1024));
    CUDA_CHECK(cudaMemcpyToSymbol(c_SBOX_INV,h_SBOX_INV,256));
}

static void host_aes256_key_expand(const uint8_t* key, uint32_t* rk) {
    const uint32_t RCON[7]={0x01000000u,0x02000000u,0x04000000u,0x08000000u,0x10000000u,0x20000000u,0x40000000u};
    for (int i=0;i<8;i++)
        rk[i]=((uint32_t)key[i*4]<<24)|((uint32_t)key[i*4+1]<<16)|((uint32_t)key[i*4+2]<<8)|key[i*4+3];
    for (int i=8;i<60;i++) {
        uint32_t t=rk[i-1];
        if (i%8==0) {
            t=(h_SBOX[(t>>16)&0xff]<<24)|(h_SBOX[(t>>8)&0xff]<<16)|(h_SBOX[t&0xff]<<8)|h_SBOX[(t>>24)&0xff];
            t^=RCON[i/8-1];
        } else if (i%8==4) {
            t=(h_SBOX[(t>>24)&0xff]<<24)|(h_SBOX[(t>>16)&0xff]<<16)|(h_SBOX[(t>>8)&0xff]<<8)|h_SBOX[t&0xff];
        }
        rk[i]=rk[i-8]^t;
    }
    for (int i=4;i<56;i++) {
        uint32_t w=rk[i];
        rk[i]=h_TD0[h_SBOX[(w>>24)&0xff]]^h_TD1[h_SBOX[(w>>16)&0xff]]
              ^h_TD2[h_SBOX[(w>>8)&0xff]] ^h_TD3[h_SBOX[w&0xff]];
    }
}

static void host_aes256_block_decrypt(const uint32_t* rk, const uint8_t* xb, const uint8_t* ct, uint8_t* pt) {
    uint32_t s0=((uint32_t)ct[0]<<24)|((uint32_t)ct[1]<<16)|((uint32_t)ct[2]<<8)|ct[3];
    uint32_t s1=((uint32_t)ct[4]<<24)|((uint32_t)ct[5]<<16)|((uint32_t)ct[6]<<8)|ct[7];
    uint32_t s2=((uint32_t)ct[8]<<24)|((uint32_t)ct[9]<<16)|((uint32_t)ct[10]<<8)|ct[11];
    uint32_t s3=((uint32_t)ct[12]<<24)|((uint32_t)ct[13]<<16)|((uint32_t)ct[14]<<8)|ct[15];
    ...
    uint32_t t0,t1,t2,t3;
    for (int r=13;r>=1;r--) {
        t0=h_TD0[(s0>>24)&0xff]^h_TD1[(s3>>16)&0xff]^h_TD2[(s2>>8)&0xff]^h_TD3[s1&0xff]^rk[r*4+0];
        t1=h_TD0[(s1>>24)&0xff]^h_TD1[(s0>>16)&0xff]^h_TD2[(s3>>8)&0xff]^h_TD3[s2&0xff]^rk[r*4+1];
        t2=h_TD0[(s2>>24)&0xff]^h_TD1[(s1>>16)&0xff]^h_TD2[(s0>>8)&0xff]^h_TD3[s3&0xff]^rk[r*4+2];
        t3=h_TD0[(s3>>24)&0xff]^h_TD1[(s2>>16)&0xff]^h_TD2[(s1>>8)&0xff]^h_TD3[s0&0xff]^rk[r*4+3];
        s0=t0;s1=t1;s2=t2;s3=t3;
    }
    t0=((uint32_t)h_SBOX_INV[(s0>>24)&0xff]<<24)|((uint32_t)h_SBOX_INV[(s3>>16)&0xff]<<16)|((uint32_t)h_SBOX_INV[(s2>>8)&0xff]<<8)|h_SBOX_INV[s1&0xff];
    t1=((uint32_t)h_SBOX_INV[(s1>>24)&0xff]<<24)|((uint32_t)h_SBOX_INV[(s0>>16)&0xff]<<16)|((uint32_t)h_SBOX_INV[(s3>>8)&0xff]<<8)|h_SBOX_INV[s2&0xff];
    t2=((uint32_t)h_SBOX_INV[(s2>>24)&0xff]<<24)|((uint32_t)h_SBOX_INV[(s1>>16)&0xff]<<16)|((uint32_t)h_SBOX_INV[(s0>>8)&0xff]<<8)|h_SBOX_INV[s3&0xff];
    t3=((uint32_t)h_SBOX_INV[(s3>>24)&0xff]<<24)|((uint32_t)h_SBOX_INV[(s2>>16)&0xff]<<16)|((uint32_t)h_SBOX_INV[(s1>>8)&0xff]<<8)|h_SBOX_INV[s0&0xff];
    t0^=rk[0];t1^=rk[1];t2^=rk[2];t3^=rk[3];
    uint32_t x0=((uint32_t)xb[0]<<24)|((uint32_t)xb[1]<<16)|((uint32_t)xb[2]<<8)|xb[3];
    uint32_t x1=((uint32_t)xb[4]<<24)|((uint32_t)xb[5]<<16)|((uint32_t)xb[6]<<8)|xb[7];
    uint32_t x2=((uint32_t)xb[8]<<24)|((uint32_t)xb[9]<<16)|((uint32_t)xb[10]<<8)|xb[11];
...
static bool host_check_multibit(const uint8_t* pw, int pw_len, const uint8_t* salt, const uint8_t* enc) {
    uint8_t salted[144];
    for (int i=0;i<pw_len;i++) salted[i]=pw[i];
    for (int i=0;i<8;i++)      salted[pw_len+i]=salt[i];
    int slen=pw_len+8;
    uint8_t key1[16],key2[16],iv[16];
    md5(salted,slen,key1);
    uint8_t tmp[160];
    ...
    for (int i=0;i<16;i++) aes_key[16+i]=key2[i];
    uint32_t rk[60];
    host_aes256_key_expand(aes_key,rk);
    uint8_t pt1[16];
    host_aes256_block_decrypt(rk,iv,enc,pt1);
    uint8_t b0=pt1[0];
    if (b0!='L'&&b0!='K'&&b0!='5'&&b0!='Q') return false;
    if (!all_b58(pt1+1,15)) return false;
    uint8_t pt2[16];
    host_aes256_block_decrypt(rk,enc,enc+16,pt2);
    return all_b58(pt2,16);
}
...
static std::vector<TokenLine> parse_tokenlist(const char* path, char delimiter) {
    ...
    while (fgets(buf,sizeof(buf),f)) {
        ...
        for (auto& tok : parts) {
            ...
            // Anchored token: ^text (pos 0), ^N^text (pos N-1, 0-indexed), text$ (last)
                ...
                if (!text.empty()&&text.back()=='$') {
                    ...
                    if (inner!=std::string::npos && inner>0) {
                        ...
                        if (is_num) {
                            ...
                            fprintf(stderr,
                                "FATAL: unsupported anchor syntax '%s'. Middle/range anchors are not implemented.\n",
                                tok.c_str());
                            exit(1);
                        }
                    } else {
                ...
                if ((int)text.size() > MAX_TOKEN_LEN) {
                    fprintf(stderr,"FATAL: token '%s' exceeds MAX_TOKEN_LEN=%d.\n", text.c_str(), MAX_TOKEN_LEN);
                    exit(1);
                }
                tl.tokens.push_back(text);
            }
            else {
                if ((int)tok.size() > MAX_TOKEN_LEN) {
                    fprintf(stderr,"FATAL: token '%s' exceeds MAX_TOKEN_LEN=%d.\n", tok.c_str(), MAX_TOKEN_LEN);
                    exit(1);
                }
    ...
    if ((int)lines.size() > MAX_LINES) {
        fprintf(stderr,"FATAL: tokenlist has %d active lines; MAX_LINES=%d.\n", (int)lines.size(), MAX_LINES);
        exit(1);
    }
    return lines;
}
...
// ---------------------------------------------------------------------------
// Host: password batch (producer/GPU handoff)
...
static uint64_t factorial_u64(int n) {
    uint64_t out = 1;
    for (int i=2;i<=n;i++) out *= (uint64_t)i;
    return out;
}
...
int main(int argc, char** argv) {
    ...
    if (restore_path) {
        ...
        printf("Restored: resuming at combo #%llu, perm #%llu, typo #%llu (%.3fT passwords done)\n",
