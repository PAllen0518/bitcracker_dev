...
class WalletMultiBit(object):
    ...
    def return_verified_password_or_false(self, orig_passwords):
        ...
        for count, password in enumerate(passwords, 1):
            ...
            b58_privkey = l_aes256_cbc_decrypt(key1 + key2, iv, encrypted_block_first)
            ...
            if b58_privkey[0] in b"LK5Q\x0a#":
                ...
                if b58_privkey[0] in b"LK5Q":  # private keys always start with L, K, or 5, or for MultiDoge Q
                    # translate strips valid b58 chars; non-empty result means an invalid char was found
                    if not b58_privkey[1:].translate(None, l_b58_valid):
                        # If another AES block is available, decrypt and check it as well to avoid false positives
                        if encrypted_block_second is not None:
                            b58_privkey = l_aes256_cbc_decrypt(key1 + key2, encrypted_block_first, encrypted_block_second)
                            if not b58_privkey.translate(None, l_b58_valid):
